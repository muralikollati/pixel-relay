...
import { pauseAccount, resumeAccount, removeAccount, requestStop } from '../utils/api';
...
const PhaseChip = ({ phase }) => {
  ...
  const s = phaseMap[phase] || { label: phase, color: '#9CA3AF', bg: 'rgba(255,255,255,0.05)' };
  return (
    <Box sx={{ display: 'inline-block', px: 1, py: 0.25, borderRadius: 1, bgcolor: s.bg, color: s.color, fontSize: 10, fontFamily: 'DM Mono, monospace' }}>
      {s.label}
    </Box>
  );
};
...
const StatusChip = ({ status }) => {
  const map = { active: 'success', paused: 'default', warning: 'warning', error: 'error' };
  return <Chip label={status} color={map[status] || 'default'} size="small" sx={{ fontFamily: 'DM Mono, monospace', fontSize: 10, height: 22 }} />;
};
  ...
  return (
    <Card>
      <CardContent sx={{ p: '16px !important' }}>
        <Typography variant="caption" sx={{ color: 'text.secondary', fontFamily: 'DM Mono, monospace', letterSpacing: '0.08em', display: 'block', mb: 1 }}>{label}</Typography>
        <Typography sx={{ fontSize: 28, fontWeight: 800, color, lineHeight: 1, mb: 0.5 }}>{value}</Typography>
    ...
    for (const [, entry] of Object.entries(allActivity || {})) {
      ...
      for (const acc of entry.accounts || []) {
        if (acc.email && acc.phase) {
          merged[acc.email] = {
            phase: acc.phase, message: acc.message || '',
            done: acc.done || 0, total: acc.total || 0,
            fromRemote: true, runBy: null,
          };
        }
      }
    }
...
export default function Dashboard({ data, refetch, onToast, worker, allActivity = {}, myActivity = {}, userRole = 'user' }) {
  ...
  const statusMap = buildStatusMap(accountStatuses, myActivity, allActivity, isAdmin);
  ...
  const runningAccounts = enriched.filter(a =>
    a.jobStatus?.phase && !['done', 'idle', 'error'].includes(a.jobStatus?.phase)
  );

  const remoteRunning = runningAccounts.filter(a => a.jobStatus?.fromRemote);

  const chartData = Array.from({ length: 7 }, (_, i) => {
    const active = accounts.filter(a => a.status === 'active' && a.stats?.trend?.length > i);
    const avg = active.length ? active.reduce((s, a) => s + a.stats.trend[i], 0) / active.length : 0;
  ...
  const handleRunOne = (account) => {
    ...
    if (liveStatus?.phase && !['done', 'idle', 'error'].includes(liveStatus.phase)) {
  ...
  const confirmAdmStop = async () => {
    ...
    try {
      ...
      onToast(`Stop signal sent to ${admStopTarget.runBy}'s session`, 'warning');
    } catch { onToast('Failed to send stop signal', 'error'); }
  ...
  const handleToggle = async (account) => {
    try {
      if (['active', 'warning'].includes(account.status)) {
        await pauseAccount(account.email); onToast(`Paused ${account.email}`);
      } else {
        await resumeAccount(account.email); onToast(`Resumed ${account.email}`);
      }
      refetch();
  ...
  const handleRemove = async () => {
    try { await removeAccount(confirmEmail); onToast(`${confirmEmail} disconnected`); refetch(); }
    catch { onToast('Failed to disconnect', 'error'); }
    finally { setConfirmEmail(null); }
  };

  const filtered = enriched.filter(a => a.email.toLowerCase().includes(search.toLowerCase()));

  return (
    <Box>
      {/* Live running banner — visible to all users when any of their accounts are running */}
      {runningAccounts.length > 0 && (
        <Box sx={{ mb: 2, px: 2, py: 1.5, borderRadius: 2, bgcolor: 'rgba(124,58,237,0.07)', border: '1px solid rgba(124,58,237,0.22)' }}>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center' }}>
            <Typography sx={{ fontSize: 11, color: '#A78BFA', fontFamily: 'DM Mono, monospace', mr: 1 }}>
            ...
            {runningAccounts.map(a => {
              ...
              return (
                <Chip key={a.email} size="small"
                  label={`${a.email.split('@')[0]} · ${j.total ? `${j.done}/${j.total} (${pct}%)` : j.phase || 'starting'}${j.runBy ? ` · by ${j.runBy}` : ''}`}
                  sx={{ bgcolor: j.fromRemote ? 'rgba(0,229,255,0.1)' : 'rgba(124,58,237,0.15)', color: j.fromRemote ? '#67E8F9' : '#C4B5FD', fontSize: 11, fontFamily: 'DM Mono, monospace' }}
                />
              );
            })}
            {isAdmin && remoteRunning.length > 0 && (
              <Chip size="small" icon={<PeopleAltIcon sx={{ fontSize: 12, ml: '6px !important' }} />}
                label={`${remoteRunning.length} from other sessions`}
                sx={{ fontSize: 10, height: 22, bgcolor: 'rgba(0,229,255,0.07)', color: '#67E8F9', border: '1px solid rgba(0,229,255,0.15)', fontFamily: 'DM Mono, monospace' }}
              />
            )}
          </Box>
        </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={6} sm={3}><StatCard label="AVG SUCCESS RATE" value={`${summary.avgSuccessRate}%`} color={rateColor(summary.avgSuccessRate)} sub="active accounts" /></Grid>
        <Grid item xs={6} sm={3}><StatCard label="EMAILS PROCESSED" value={summary.totalEmails.toLocaleString()} color="#00E5FF" sub="all time" /></Grid>
        <Grid item xs={6} sm={3}><StatCard label="ACTIVE ACCOUNTS"  value={`${summary.activeAccounts}/${summary.totalAccounts}`} color="#7C3AED" sub="of total connected" /></Grid>
        <Grid item xs={6} sm={3}><StatCard label="NEEDS ATTENTION"  value={summary.warnings} color={summary.warnings > 0 ? '#EF4444' : '#10B981'} sub="warning or error" /></Grid>
      </Grid>
        ...
        <CardContent>
          ...
          {accounts.length === 0 ? (
            <Box sx={{ height: 180, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
              ...
              <Typography color="text.secondary" variant="body2">No accounts connected yet</Typography>
            </Box>
          ) : (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -22 }}>
                ...
                <XAxis dataKey="label" tick={{ fontSize: 9, fill: '#4B5563' }} tickLine={false} axisLine={false} />
                <YAxis domain={[yMin, 100]} tick={{ fontSize: 9, fill: '#4B5563' }} tickLine={false} axisLine={false} />
                <RTooltip contentStyle={{ background: '#0F1117', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 11 }} itemStyle={{ color: '#00E5FF' }} />
                <Area type="monotone" dataKey="success" stroke="#00E5FF" strokeWidth={2} fill="url(#agrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </CardContent>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2, flexWrap: 'wrap', gap: 1 }}>
            <Typography variant="caption" sx={{ color: 'text.secondary', fontFamily: 'DM Mono, monospace', letterSpacing: '0.08em', fontSize: 11 }}>
              CONNECTED ACCOUNTS
              {isAdmin && <Box component="span" sx={{ ml: 1, color: '#67E8F9' }}>— all users visible</Box>}
            </Typography>
            <TextField
              size="small" placeholder="Filter accounts..." value={search}
              onChange={e => setSearch(e.target.value)}
              InputProps={{
                startAdornment: <InputAdornment position="start"><SearchIcon sx={{ fontSize: 16, color: 'text.disabled' }} /></InputAdornment>,
                sx: { fontSize: 12, fontFamily: 'DM Mono, monospace', bgcolor: 'rgba(255,255,255,0.03)' },
              }}
          ...
          {filtered.length === 0 ? (
            <Box sx={{ textAlign: 'center', py: 5 }}>
              <Typography color="text.disabled" variant="body2">
                {accounts.length === 0 ? 'No accounts connected.' : 'No accounts match your filter.'}
              </Typography>
            </Box>
          ) : (
            <TableContainer component={Paper} elevation={0} sx={{ bgcolor: 'transparent', overflowX: 'auto' }}>
              <Table size="small" sx={{ minWidth: 640 }}>
                <TableHead>
                  <TableRow>
                    {['Account', 'Owner', 'Rate', 'Emails', 'Beacons', 'Trend', 'Status', 'Actions'].map(h => (
                      <TableCell key={h} sx={{ color: 'text.disabled', fontSize: 10, fontFamily: 'DM Mono, monospace', letterSpacing: '0.08em', py: 1 }}>
                        {h.toUpperCase()}
                      </TableCell>
                    ))}
                  </TableRow>
                  ...
                  {filtered.map(a => {
                    ...
                    return (
                      <TableRow key={a.email}
                        sx={{
                          bgcolor: isActive ? (fromRemote ? 'rgba(0,229,255,0.03)' : 'rgba(124,58,237,0.04)') : 'transparent',
                      ...
                      >
                        {/* Account + live status */}
                        <TableCell sx={{ py: 1.5 }}>
                          <Typography sx={{ fontSize: 12, fontFamily: 'DM Mono, monospace' }}>{a.email}</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.5, flexWrap: 'wrap' }}>
                            <PhaseChip phase={jobSt.phase} />
                            {jobSt.runBy && (
                              <Box sx={{ px: 1, py: 0.2, borderRadius: 1, bgcolor: 'rgba(0,229,255,0.08)', color: '#67E8F9', fontSize: 9, fontFamily: 'DM Mono, monospace' }}>
                                by {jobSt.runBy}
                              </Box>
                            )}
                            {jobSt.message && (
                              <Typography variant="caption" color="text.disabled" sx={{ fontSize: 9 }}>{jobSt.message}</Typography>
                            )}
                          </Box>
                          {isActive && jobSt.total > 0 && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.75 }}>
                              <LinearProgress variant="determinate" value={progress}
                                sx={{ flex: 1, height: 3, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.06)', '& .MuiLinearProgress-bar': { bgcolor: fromRemote ? '#00E5FF' : '#7C3AED' } }} />
                              <Typography variant="caption" sx={{ fontSize: 9, color: fromRemote ? '#00E5FF' : '#7C3AED', fontFamily: 'DM Mono, monospace', flexShrink: 0 }}>
                        ...
                        <TableCell sx={{ py: 1.5 }}>
                          <Typography sx={{ fontSize: 11, color: 'text.disabled', fontFamily: 'DM Mono, monospace' }}>{a.owner || '—'}</Typography>
                        </TableCell>

                        <TableCell sx={{ py: 1.5 }}>
                          <Typography sx={{ fontSize: 14, fontWeight: 700, color: rateColor(rate), fontFamily: 'DM Mono, monospace' }}>{rate}%</Typography>
                        </TableCell>

                        <TableCell sx={{ py: 1.5 }}>
                          <Typography sx={{ fontSize: 12, color: 'text.secondary', fontFamily: 'DM Mono, monospace' }}>
                        ...
                        <TableCell sx={{ py: 1.5 }}><StatusChip status={a.status} /></TableCell>
                          ...
                          <Box sx={{ display: 'flex', gap: 0.5 }}>
                            ...
                            {canStop ? (
                              <Tooltip title="Stop after current batch">
                                <IconButton size="small" onClick={() => handleStopOne(a.email)}
                                  sx={{ color: '#EF4444', bgcolor: 'rgba(239,68,68,0.12)', borderRadius: 1.5, '&:hover': { bgcolor: 'rgba(239,68,68,0.22)' } }}>
                              ...
                              fromRemote && isAdmin ? (
                                <Tooltip title={`Signal ${jobSt.runBy || 'session'} to stop ${a.email}`}>
                                  <IconButton size="small" onClick={() => setAdmStopTarget({ email: a.email, runBy: jobSt.runBy || '' })}
                                    sx={{ color: '#F59E0B', bgcolor: 'rgba(245,158,11,0.12)', borderRadius: 1.5, '&:hover': { bgcolor: 'rgba(245,158,11,0.22)' } }}>
                                    <StopIcon sx={{ fontSize: 14 }} />
                                  </IconButton>
                                </Tooltip>
                              ) : (
                                <Tooltip title={fromRemote ? `Running by ${jobSt.runBy || 'another session'}` : 'Stop All in progress — use toolbar'}>
                                  <span>
                                    <IconButton size="small" disabled
                                      sx={{ color: '#EF4444', bgcolor: 'rgba(239,68,68,0.06)', borderRadius: 1.5, '&.Mui-disabled': { opacity: 0.35 } }}>
                                      <StopIcon sx={{ fontSize: 14 }} />
                                    </IconButton>
                                  </span>
                                </Tooltip>
                              ...
                              <Tooltip title={
                                runAllMode ? 'Run All in progress — use Stop All in toolbar' :
                                running    ? 'Another account is running' :
                                !['active','warning'].includes(a.status) ? 'Resume account first' :
                                `Run ${a.email}`
                              }>
                                <span>
                                  <IconButton size="small" onClick={() => handleRunOne(a)} disabled={!canRun}
                                    sx={{ color: '#00E5FF', bgcolor: 'rgba(0,229,255,0.08)', borderRadius: 1.5, '&:hover': { bgcolor: 'rgba(0,229,255,0.18)' }, '&.Mui-disabled': { opacity: 0.25 } }}>
                            ...
                            <Tooltip title={!canPause ? (isActive ? 'Stop the run first before pausing' : '') : (['active','warning'].includes(a.status) ? 'Pause account' : 'Resume account')}>
                              <span>
                            ...
                            <Tooltip title={!canDelete ? 'Cannot delete while account is processing — stop the run first' : 'Disconnect account'}>
                              <span>
                                <IconButton size="small" onClick={() => canDelete && setConfirmEmail(a.email)} disabled={!canDelete}
                                  sx={{ color: '#EF4444', bgcolor: 'rgba(239,68,68,0.08)', borderRadius: 1.5, '&:hover': { bgcolor: 'rgba(239,68,68,0.18)' }, '&.Mui-disabled': { opacity: 0.3 } }}>
