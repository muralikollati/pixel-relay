...
router.get('/google/callback', async (req, res) => {
  ...
  try {
    ...
    const maxAccountsPerUser = ConfigStore.get().maxAccountsPerUser || 10;
    const existingCount = (require('../services/db').db
      .prepare('SELECT COUNT(*) as cnt FROM accounts WHERE owner=?')
      .get(owner)?.cnt) || 0;
