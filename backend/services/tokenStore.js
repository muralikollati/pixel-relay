...
const TokenStore = {
  save(email, tokenData, ownerUsername) {
    const tokenObj = {
    ...
    const existing = db.prepare('SELECT stats, status FROM accounts WHERE email=?').get(email);
    db.prepare(`
      INSERT INTO accounts (email,owner,tokens,status,stats,added_at,updated_at)
